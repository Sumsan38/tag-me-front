...
마인드맵은 두 가지 데이터로 구성된다. **노드**(태그 자체)는 `diary_tags` · `post_tags` · `user_tag_interactions`를 UNION하여 내가 사용하거나 인터랙션한 태그 목록을 조회한다. **엣지**(태그 간 연결)는 `tag_co_occurrences`에서 태그 쌍의 출현 횟수를 집계하여 가중치와 출처를 함께 반환한다.

| 기능 | 설명 |
|------|------|
| 태그 시각화 | 주별 / 월별 / 연도별 입력 태그를 마인드맵으로 표시 |
| **태그 출처 시각화** | 직접 작성 태그(진한 색) vs 좋아요·댓글 태그(연한 색·아이콘 구분). 노드 색상은 `user_tag_interactions.source` 및 `diary_tags` / `post_tags` 출처로 결정, 엣지 스타일은 `tag_co_occurrences.source_type`으로 결정 |
| 태그 클릭 | 해당 태그가 포함된 일기(`diary_tags`)·게시글(`post_tags`)·인터랙션(`user_tag_interactions`) 목록 노출, 출처별 탭 구분 |
...
```
[diary 인덱스]                    [feed 인덱스]
- title        (weight: 3)       - title        (weight: 3)
- content      (weight: 1)       - content      (weight: 1)
- tags[]       (weight: 2)       - tags[]       (weight: 2)
- user_id      (private 필터)    - is_public    (공개 여부 필터)
- created_at   (기간 필터)       - created_at   (기간 필터)
```
...
| 표시값(display) | `#` + 원본 의미를 유지한 정규화 문자열 |
| 저장값(canonical) | 검색/집계/중복 체크에 사용하는 소문자 정규화 값 |

예시:
- 입력 `# 여행 ` → 저장 `여행`, 표시 `#여행`
- 입력 ` Burn Out ` → 저장 `burn out`, 표시 `#burn out`
- 입력 `러닝!!!` → 저장 `러닝`, 표시 `#러닝`
...
| 재가입 정책 | 탈퇴 후 동일 이메일 재가입 허용 여부는 운영 정책으로 별도 결정하되, 기본은 30일 유예 후 허용 |
| 탈퇴 후 소셜 재인증 | 유예 기간 중 재로그인 시 계정 복구 여부를 정책으로 명시해야 하며, 초기 버전은 복구 미지원 권장 |
...
| **Feed** | 공개 게시글, 좋아요, 댓글 | `Post`, `Like`, `Comment` |
...
| **게시글 좋아요 수 캐시** | String | `post:likes:{postId}` | **캐시 미스 시 DB 동기화** |
...
| source_type | ENUM | `'diary'` \| `'post'` \| `'like'` \| `'comment'` |
| source_id | BIGINT | diary_id 또는 post_id |
...
> | `post` | `PostCreatedEvent` | 이벤트 payload의 태그 목록 | FeedService |
> | `like` | `LikeAddedEvent` | 이벤트 payload의 태그 목록 (post_tags에서 조회 후 payload에 포함) | FeedService |
...
>
> **태그 수정 처리 (`DiaryUpdatedEvent` / `PostUpdatedEvent`)**
>
...
> - **태그 제거 시**: `tag_co_occurrences`에서 `source_id = diaryId/postId AND (tag_id_a = removedTagId OR tag_id_b = removedTagId)` 조건으로 `is_deleted = true`. `user_tag_interactions`에서 `source_id = diaryId/postId AND tag_id = removedTagId` 행 `is_deleted = true`
>
...
| tag_id | BIGINT FK → tags | |
| source_type | ENUM | `'diary'` \| `'post'` \| `'like'` \| `'comment'` |
| source_id | BIGINT | source_type에 따라 diary_id 또는 post_id |
...
> | `post` | `PostCreatedEvent` | post_id | 게시글 태그 각 1행. user_id = 작성자 |
...
> | 케이스 | 처리 방식 |
> |--------|---------|
> | 좋아요 취소 | `source_type='like' AND source_id=postId AND user_id=userId` 행 → `is_deleted=true` |
> | 댓글 삭제 (`CommentDeletedEvent`) | `source_type='comment' AND source_id=postId AND user_id=userId AND created_at=commentCreatedAt` 행 → `is_deleted=true`. `tag_co_occurrences`도 동일 조건으로 소프트 삭제 |
...
> | 게시글 삭제 | `source_type='post' AND source_id=postId` 행 → `is_deleted=true` |
> | 게시글 수정 — 태그 제거 | `source_type='post' AND source_id=postId AND tag_id=removedTagId` 행 → `is_deleted=true` |
> | 게시글 수정 — 태그 추가 | 추가된 태그 행 신규 삽입 (`PostUpdatedEvent.addedTags` 기준) |
>
> **primarySource 결정 규칙 (애플리케이션 레이어)**
>
> `primarySource`는 DB 컬럼이 아니라 노드 쿼리 결과(`diary_count`, `post_count`, `like_count`, `comment_count`)를 기반으로 응답 생성 시점에 계산하는 파생값이다.
> 결정 기준: count가 가장 높은 source_type을 채택. 동점 시 `diary > post > comment > like` 우선순위를 따른다.
...
>
> **Redis 역할**: `post:likes:{postId}` String 키에 좋아요 수를 캐싱해 피드 목록 조회 시 DB 집계 쿼리를 대체한다. 좋아요/취소 시 DB 저장과 동시에 `INCR` / `DECR`로 카운터를 갱신한다. 캐시 미스 시 DB에서 `COUNT(*)` 후 Redis에 적재한다.
...
> - **`user_tag_interactions`는 모든 출처(diary/post/like/comment)의 태그 단건 이력을 통합 저장**한다. 마인드맵 노드 집계(`GROUP BY tag_id`)와 태그 클릭 상세 탭을 단일 테이블 쿼리로 처리할 수 있다. `tag_co_occurrences`는 태그 쌍 관계(엣지 가중치·출처 필터)를 담당하며 두 테이블은 항상 동일 이벤트에서 함께 기록된다
> - `circles`와 `challenges` 모두 단일 태그가 아닌 `circle_tags` / `challenge_tags`를 통한 **복수 태그 집합**으로 정의된다
> - `likes`는 DB 테이블로 영속 저장하고, Redis `post:likes:{postId}`로 카운터를 캐싱한다. 좋아요/취소 시 DB와 Redis를 동시에 갱신하며, 캐시 미스 시 DB에서 COUNT 후 Redis에 적재한다
...
| **Phase 2** — 핵심 기능 개발 | 3~6주 | 회원가입/로그인, 일기 CRUD, 태그 시스템, 자동완성, **통합 검색 (Elasticsearch 인덱싱)** |
| **Phase 3** — 소셜 기능 | 7~10주 | 공개 피드, 이미지 업로드 (S3 Pre-signed URL), 좋아요/댓글, 써클 기능, **EKS 클러스터 구축 및 기본 배포** |
...
- **M2** (6주차): 핵심 기능(일기 + 태그 + 검색) 동작 확인
- **M3** (14주차): 소셜 피드 + 마인드맵 통합 완료
...
**6주차 — Search 도메인 + 통합 검색 + 일기 완성 후 피드 추천**
- [ ] 통합 검색 API (`GET /api/v1/search?q=`) 구현
- [ ] 제목(×3) / 태그(×2) / 본문(×1) 필드 가중치 쿼리 구현
- [ ] 검색 결과 하이라이팅 구현
- [ ] 기간·태그 검색 필터 구현
- [ ] `DefaultSearchService` 통합 테스트 작성 (H2 + ES MockBean)
- [ ] `DiaryCreatedEvent` 리스너 → 태그 목록 추출 구현
...
**7주차 — Feed 도메인**
- [ ] `Post` Aggregate, `Comment` / `Like` Entity 구현 + 도메인 테스트 케이스 작성
- [ ] 게시글 작성/조회/삭제 API 구현 (`POST/GET/DELETE /api/v1/posts`)
- [ ] 팔로우 기반 피드 조회 API (Cursor 페이지네이션) 구현
- [ ] 전체 공개 피드 조회 API 구현
- [ ] `DefaultPostService` 통합 테스트 작성 (H2)

**8주차 — 좋아요 / 댓글 + 인터랙션 태그 기록**
- [ ] 좋아요 API (`POST/DELETE /api/v1/posts/{id}/likes`) 구현
- [ ] 댓글 API (`POST/GET/DELETE /api/v1/posts/{id}/comments`) 구현
...
- [ ] `diary_tags` + `post_tags` + `user_tag_interactions` UNION 집계 쿼리 구현
