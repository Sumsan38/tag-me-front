...
| **🤝 1:1 태그 친구 매칭** | 동일 태그로 꾸준히 기록 중인 유저와 1:1 매칭, 짧은 응원 메시지(100자) + 1:1 채팅(채팅 인프라 재활용) 가능. 최대 수 무제한, 재매칭 제한 없음(랜덤성 분산). 매칭 알고리즘: 태그 겹침 + 유저 활동 최신순 + 랜덤성 |
...
#### 제재 상태에 따른 기능 제한 (2026-04-07 확정)

| 제재 상태 | 조회 | 쓰기(글/댓글/좋아요/채팅) | 로그인 | 비고 |
|----------|------|-------------------------|--------|------|
| ACTIVE (정상) | 가능 | 가능 | 가능 | |
| WARNING | 가능 | 가능 | 가능 | 경고 알림만 발송 |
| HIDE | 가능 | 가능 | 가능 | 제재 대상 콘텐츠만 타인에게 비노출 |
| SUSPEND | 가능 | **차단** (403) | 가능 | 정지 안내 화면 + 해제일시 제공 |
| BAN | 불가 | 불가 | **차단** (401) | 영구 정지 안내 화면만 노출 |
...
| 보존 기간 | 90일 (읽은/안 읽은 동일). 초과 시 **hard delete** (물리 삭제). 배치 매일 자정 실행 |
...
| 알림 | 90일 보관 후 hard delete (읽은/안 읽은 동일, 매일 자정 배치) |
...
| 응원 메시지 | 매칭 종료 후 7일 보존, 이후 soft delete. 신고/분쟁 대응 기간 고려 |

### 2.12 태그 친구 운영 정책 (2026-04-07 확정)

1:1 태그 친구 기능은 소규모 메시징 성격이 있으므로 운영 범위와 안전 정책을 명시한다.
...
| 매칭 알고리즘 | 태그 겹침 + 유저 활동 최신순 + 랜덤성 (동일 상대 반복 매칭 방지) |
| 재매칭 제한 | 없음 (랜덤성으로 분산). 차단 관계만 제외 |
| 소통 수단 | 응원 메시지(100자) + 1:1 채팅 (17주차 채팅 인프라 재활용) |
| 응원 메시지 보존 | 매칭 종료 후 7일, 이후 soft delete |
...
### 2.13 신고/차단/제재 정책 (2026-04-07 확정)

#### 차단 (`user_blocks`)

| 항목 | 정책 |
|------|------|
| 차단 범위 | 피드/댓글 안 보임 + 자동 언팔로우 + 팔로우 불가 |
| 차단 조회 | 양방향 (차단한 쪽 + 차단당한 쪽 모두 조회 가능, 관리자 용도) |
| 태그 친구 연동 | 차단 시 기존 태그 친구 매칭 해제, 재매칭 불가 |

#### 신고 (`user_reports`)

| 항목 | 정책 |
|------|------|
| 신고 유형 | `SPAM`, `HARASSMENT`, `INAPPROPRIATE_CONTENT`, `OTHER` |
| 신고 대상 | polymorphic — `target_type` (USER / FEED / COMMENT), `target_id` |
| 처리 상태 | `PENDING` → `RESOLVED` (제재 조치) / `DISMISSED` (문제 없음) |
| 자동 제재 | 동일 대상 10건 누적 시 자동 제재 발동 + 관리자 사후 확인 |
| 관리자 처리 | 관리자 전용 REST API로 신고 목록 조회/처리 (백오피스 웹 추후 검토) |

#### 제재 (`user_sanctions`) — 단계별 에스컬레이션

| 단계 | 기본 기간 | 동작 |
|------|----------|------|
| **WARNING** | 7일 (기록용) | 알림으로 경고 전달, 기능 제한 없음 |
| **HIDE** | 7일 | 해당 콘텐츠(피드/댓글)만 타인에게 비노출, 본인은 정상 이용 |
| **SUSPEND** | 7일 | 로그인 가능, 읽기 전용 (글쓰기/댓글/좋아요/채팅 차단), 정지 안내 화면 노출 |
| **BAN** | 영구 | 로그인 차단, 접속 시 영구 정지 안내 화면만 노출 |

> 각 단계별 기본 기간은 `SanctionType` Enum으로 관리. 관리자가 건별로 기간 조정 가능.

#### 제재 구현 영역 (백엔드 중심)

| 제재 단계 | 백엔드 구현 | 프론트엔드 구현 |
|----------|-----------|---------------|
| WARNING | 알림 생성 (`SANCTION_WARNING` 타입) | 알림 수신 시 경고 모달 노출 |
| HIDE | 피드/댓글 조회 API에서 해당 콘텐츠 필터링 제외 | 변경 없음 (데이터가 안 내려옴) |
| SUSPEND | 쓰기 API 호출 시 403 + 제재 정보 응답 (해제일시 포함) | 403 응답 감지 → 정지 안내 화면 노출, 쓰기 UI 비활성화 |
| BAN | JWT 검증 단계에서 차단 → 401 + 영구 정지 응답 | 401 응답 감지 → 영구 정지 안내 화면 리다이렉트 |
